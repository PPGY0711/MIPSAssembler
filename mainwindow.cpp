...
void MainWindow::showDisplay(){
    ...
    //                printf("char %d: %02x%02x\n",i-0x3000,cs[0],cs[1]);
        ...
        ui->PCWindow->append(QString::fromLocal8Bit(this->mainWindowStr.c_str()));
    }
    else{
        //系统调用1打印数字
        int* iptr = (int*)this->mpc.intRetValuePtr;
        this->mainWindowStr = dex2str((*iptr));
        ui->PCWindow->append(QString(this->mainWindowStr.c_str()));
    }
